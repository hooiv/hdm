...
const PluginEditor: React.FC = () => {
    ...
    const handleSelectPlugin = async (name: string) => {
        ...
        try {
            setIsLoading(true);
            const content = await invoke<string>('get_plugin_source', { filename: name });
            setCode(content);
            setSelectedPlugin(name);
            setIsDirty(false);
        } catch (e) {
            console.error(e);
            alert("Failed to load plugin source: " + e);
        } finally {
            setIsLoading(false);
        }
    };
    ...
    const handleSave = async () => {
        ...
        try {
            await invoke('save_plugin_source', { filename: selectedPlugin, content: code });
            setIsDirty(false);
            // alert("Saved!"); 
            // Better UI than alert
            setOutput(prev => [`[${new Date().toLocaleTimeString()}] Saved ${selectedPlugin}.lua`, ...prev]);
        } catch (e) {
            alert("Save failed: " + e);
        }
    };
        ...
        <div className="h-full flex overflow-hidden glass-panel rounded-2xl mx-4 my-2 border border-white/10 shadow-2xl backdrop-blur-xl bg-slate-900/60">
            ...
            <div className="w-64 border-r border-white/5 flex flex-col bg-slate-900/40">
                <div className="p-4 border-b border-white/5 flex justify-between items-center">
                    ...
                    <button onClick={handleCreate} className="p-1 hover:bg-white/10 rounded-full transition-colors text-emerald-400">
                        <Plus size={18} />
                    </button>
                </div>
                        ...
                        >
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-2">
                                    <FileCode size={16} className={selectedPlugin === p.name ? 'text-cyan-400' : 'text-slate-500'} />
                            ...
                            <div className="text-xs text-slate-500 mt-1 pl-6">v{p.version}</div>
                        </div>
                    ))}

                    {plugins.length === 0 && (
                        <div className="text-center p-8 text-slate-600 italic">No plugins found</div>
                    )}
                </div>
            </div>
            ...
            <div className="flex-1 flex flex-col relative bg-[#1E1E1E]">
                ...
                <div className="h-12 border-b border-white/5 flex items-center justify-between px-4 bg-[#252526]">
                    <div className="flex items-center gap-4">
                        {selectedPlugin ? (
                            <>
                                <span className="text-sm text-slate-300 font-mono">{selectedPlugin}.lua</span>
                                {isDirty && <span className="text-amber-400 text-xs bg-amber-400/10 px-2 py-0.5 rounded-full">● Unsaved</span>}
                            </>
                        ) : (
                            <span className="text-slate-500 text-sm italic">Select a plugin to edit</span>
                        )}
                    </div>
                ...
                <div className="h-32 border-t border-white/5 bg-[#1E1E1E] flex flex-col">
                    <div className="px-4 py-1 bg-[#252526] text-xs font-bold text-slate-500 uppercase tracking-wider flex justify-between">
                        <span>Output</span>
                        <button onClick={() => setOutput([])}><Trash2 size={12} /></button>
                    </div>
                    <div className="flex-1 overflow-y-auto p-2 font-mono text-xs space-y-1 custom-scrollbar">
                        {output.map((line, i) => (
                            <div key={i} className="text-slate-400 border-b border-white/5 last:border-0 pb-1">{line}</div>
                        ))}
                        {output.length === 0 && <div className="text-slate-700 italic px-2">No output</div>}
                    </div>
                </div>
            </div>
        </div>
