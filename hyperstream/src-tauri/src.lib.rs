    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            app.handle().manage(AppState { 
                 ...
                 mirror_aggregator: Arc::new(crate::network::mirror_aggregator::MirrorAggregator::new()),`n                  parallel_mirror_retry: Arc::new(crate::parallel_mirror_retry::ParallelMirrorRetryManager::new(Default::default())),
            });
            ...
            // Start Mirror Aggregator discovery loop
            {
                ...
                tauri::async_runtime::spawn(async move {
                    aggregator.start_loop(app_handle).await;`n                });`n            }`n`n            // Start Parallel Mirror Retry monitoring loop`n            {`n                let app_handle = app.handle().clone();`n                let retry_manager = app.handle().state::<AppState>().parallel_mirror_retry.clone();`n                tauri::async_runtime::spawn(async move {`n                    retry_manager.start_loop(app_handle).await;
                });
            }
